...
export default {
  ...
  created() {
    const data = '中国加油,武汉加油,'
      .repeat(20)
      .slice(0, -1)
      .split(',');
    this.data = data.map((text, index) => {
      return text + index
    })
  ...
  methods: {
    ...
    animate() {
      ...
      this.rotateY();
      this.move();
      this.animateObj = window.requestAnimationFrame(this.animate);
    },
    rotateX() {
    ...
    handleMouseover(event) {
      ...
      const textIndex = this.data.findIndex(item => item === text)
